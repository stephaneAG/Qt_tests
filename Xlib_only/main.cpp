...
unsigned long white_pixel, black_pixel; // black & white 'colors'

/* current dirty hack*/
unsigned long int DesktopWinId; // the "Desktop" Window Id ( will be fetched using the 'WinIdGetter.sh' bash script in final usage )
/* TO DO */
// the current screen/root window width
// the current screen/root window height
...
int main(int argc, char *argv[])
{
	...
	// == dirty hack == // still hardcoded, by WIP v2
        DesktopWinId = 0x2000004; // hardcoded by now
	
	white_pixel = WhitePixel(display, screen);
	black_pixel = BlackPixel(display, screen);
	
	//win = XCreateSimpleWindow(display, root, 0, 0, 100, 90, 2, black_pixel, white_pixel); // was working for the 'dirty hack' v1: now trying to add stuff directly to root window ( see below )
	win = XCreateSimpleWindow(display, DesktopWinId, 0, 0, 100, 90, 2, black_pixel, white_pixel);
	...
	// == dirty hack == // WORKS ! ;D
	//DesktopWinId = 0x2000004; // hardcoded by now
	//XReparentWindow(display, win, DesktopWinId, 0, 0); // WIP: trying 'dirty hack' without reparenting but instead directly adding stuff to the root window ( see above )
