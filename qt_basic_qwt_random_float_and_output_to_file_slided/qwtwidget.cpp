...
void Qwtwidget::fakeReceive()
{
  ...
  // trying an "alternate mixed way" to do it .. like Beckham ? ;p (NO ! like P5! :D )
  out << "Current size of QVector:" << data_x->size() << endl;
  if( data_x->size() < 120 )
    addpoint(i); //we add a point to the graph
  else {
    //clearAndAddpoint(i); // clear and add point to the graph
    // "slideDown" the QVector(s) holding the data
    //slideDownVector(data_x); // bug
    //slideDownVector(data_y); // as well

    // the following "slideDown" seems to work, but also actually do ( visually ) the exact opposite of what I'm looking after ( .. )
    //for( int idx = 0; idx < data_x->size() - 1; idx++ )
    //  (*data_x)[idx] = (*data_x)[idx+1]; // result in the opposite to what I'm looking for
    for( int idy = 0; idy < data_y->size() - 1; idy++ )
      (*data_y)[idy] = (*data_y)[idy+1];
    ...
    // shrink the "size()" to one ? ( resize the vector ?
    //data_x->resize(data_x->size() -2); // remove last element from QVector ( should shrink it's size by 1 )
    data_y->resize(data_y->size() -1); // remove last element from QVector ( should shrink it's size by 1 )
    //curve->setSamples(*data_x,*data_y);
    //plot->replot(); // if it's the reason why, I am just a mess ..

    addpoint(i);// add a point to the graph
  }
...
/* modified fcns extracted from "qt_vectors_test" */
/*
void Qwtwidget::slideDownVector(QVector<double>*& theVector)
{
  QVector<double> *theActualVector = *theVector;
  for( int i = 0; i < theActualVector->size() - 1; i++ )
    theActualVector[i] = theActualVector[i+1];
  // shrink the "size()" to one ? ( resize the vector ?
  theActualVector->resize(theActualVector->size() -1); // remove last element from QVector ( should shrink it's size by 1
}
*/
...
void Qwtwidget::slideDownVector(QVector<double>* theVector)
{
  ...
  for( int i = 0; i < theVector->size() - 1; i++ )
    ...
    // shrink the "size()" to one ? ( resize the vector ?
  theVector->resize(theVector->size() -1); // remove last element from QVector ( should shrink it's size by 1   
}
